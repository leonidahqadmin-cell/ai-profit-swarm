"""
AI Profit Swarm v4.6 - Configured with Jaeger Backend

This version sends OpenTelemetry traces to Jaeger for visualization.

How to use:
1. Start Jaeger (see instructions below)
2. Run this script
3. Open Jaeger UI at http://localhost:16686
4. Search for traces named "full_cycle" or agent names

This gives you a beautiful UI to explore traces across agents.
"""
...
# Configure OpenTelemetry to send traces to Jaeger via OTLP

trace.set_tracer_provider(TracerProvider())

# Jaeger OTLP endpoint (default when running Jaeger with Docker)
...
print("=== AI PROFIT SWARM v4.6 (Jaeger Backend) ===\n")
print("Traces will be sent to Jaeger at http://localhost:4317")
print("Open Jaeger UI at: http://localhost:16686\n")

STATE_FILE = "swarm_state_v4_6.json"
...
def run_daily_cycle():
    with tracer.start_as_current_span("full_cycle") as span:
        print(f"Starting v4.6 cycle at {datetime.now().strftime('%Y-%m-%d %H:%M')}\n")
        ...
        print("\n" + "="*75)
        print("CYCLE COMPLETE - v4.6 (Jaeger)")
        print("="*75)
        print(f"Tasks executed: {len(state['tasks'])}")
        print("\nTraces have been sent to Jaeger!")
        print("Open http://localhost:16686 to view traces.")
        print("Search for service name or trace ID.")
